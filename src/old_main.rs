...
// Serialize hashmaps by converting complex keys to strings
...
// Pathfind to the enemy's base using breadth-first search
...
// Create a tower given a team color, tower type string, and position
impl Tower {
    // The tower_type string determines the tower's stats
    fn new(position: Position, team_color: TeamColor, tower_type: String) -> Result<Tower, String> {
        ...
        // If the tower_type string isn't the name of a valid tower type, return an error
        match tower_stats {
            Ok(stats) => Ok(Tower {
                uid: generate_uid(),
                position,
...
impl GameState {
    ...
    fn add_tower(&mut self, tower_type: String, position: Position, team_color: TeamColor) -> () {
        ...
        // Create the tower
        // TODO: this seems to be called again unnecessarily in perform_builder_action()
        match Tower::new(position, team_color, tower_type) {
            Ok(tower) => {
                // Get the tower's entity key so we can insert it into the map
                let key = EntityKey {
                    uid: tower.uid,
                    entity_type: EntityType::Tower,
                };
                // try to insert the tower into the map
                ...
                // try to insert the tower into the list of towers
    ...
    fn move_mercenary(&mut self, mercenary: &mut Mercenary) -> Result<(),String> {
        ...
        match self.entity_position.remove(&mercenary.position) {
            ...
            None => return Err("entity_position map is desynced".to_string()),
        }
        // push teammate forward if possible
        match self.entity_position.insert(desired, key) {
            None => (),
            Some(_) => return Err("Replacing an entity in the entity_position map".to_string()),
        }
...
fn move_mercenaries(game_state: &mut GameState) -> () {
    // Some mercenaries will try to move into the same position
    // Store desired positions as a map from the position to a list of mercenaries who want to move there
    let mut desired: HashMap<Position, Vec<&Mercenary>> = HashMap::new();

    // Populate the conflicts map by looping through all mercenaries
    for merc in game_state.player_state_red.mercenaries.values_mut() {
        // See documentation of can_move_this_turn in the Mercenary struct
        merc.can_move_this_turn = true;
        let desire = &merc.get_desired_position();
        // Is this area contested by another mercenary?
        match desired.get_mut(desire) {
            // Yes. Put this mercenary in conflict with the other one
            Some(other) => other.push(merc),
            // No. Make a new list of mercenaries who want this position, containing just the current mercenary
            None => {
                let mut new = Vec::new();
                new.push(merc);
                desired.insert(merc.position, new);
            }
        }
    }
    // Do the same thing for blue mercenaries
    for merc in game_state.player_state_blue.mercenaries.values_mut() {
        merc.can_move_this_turn = true;
        let desire = &merc.get_desired_position();
        match desired.get_mut(desire) {
            Some(other) => other.push(merc),
    ...
    // Now, move mercenaries who aren't in conflict
    for conflict in desired {
        if conflict.1.len() == 1 {
            let merc = conflict.1[0];
            game_state.move_mercenary(merc);
        }
    }
}
