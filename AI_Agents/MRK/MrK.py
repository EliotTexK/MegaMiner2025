...
class AIAction:
    ...
    def __init__(
        ...
        self.action = action.lower().strip()  # "build", "destroy", or "not>
        self.x = x
        self.y = y
        self.tower_type = tower_type.strip()
        self.merc_direction = merc_direction.upper().strip()  # "N", "S", ">
...
# Return a list of strings representing available mercenary queue direction
...
def get_available_build_spaces(game_state: dict, team_color: str):
    ...
    return result# team_color should be 'r' or 'b'
...
class Agent:
	def initialize_and_set_name(self, initial_game_state: dict, team_color: str):
	...
	def do_turn(self, game_state: dict) -> AIAction:
        # -- YOUR CODE BEGINS HERE --
        # Competitors: For your convenience, it's recommended that you use helper functions
		...
		if random.random() < provoke_chance:
			return AIAction("nothing", 0, 0, provoke_demons=True)

        # Building Strategy
		if turn in [0, 1, 4, 5, 9] and build_spaces:
			house_x, house_y = random.choice(build_spaces)
			self.num_houses += 1
			return AIAction("build", house_x, house_y, 'house')
		elif turn in [2, 6] and build_spaces:
			cannon_x, cannon_y = random.choice(build_spaces)
			self.num_cannons += 1
			return AIAction("build", cannon_x, cannon_y, 'cannon')
		elif turn == 10 and build_spaces:
			minigun_x, minigun_y = random.choice(build_spaces)
			self.num_miniguns += 1
			return AIAction("build", minigun_x, minigun_y, 'minigun')
		elif turn in [3, 8] and build_spaces:
			crossbow_x, crossbow_y = random.choice(build_spaces)
			self.num_crossbows += 1
			return AIAction("build", crossbow_x, crossbow_y, 'crossbow')
		elif turn in [15,30] and build_spaces:
			church_x, church_y = random.choice(build_spaces)
			self.num_churches += 1
			return AIAction("build", church_x, church_y, 'church')
		elif q_directions:
...
# Competititors: Altering code below this line will result in disqualificat>
