...
# pick the best merc queue direction toward the enemy base
def get_best_queue_direction(game_state: dict, team_color: str, queue_directions: list, opponentBase_x: int, opponentBase_y: int) -> str:
    if not queue_directions:
        return ""
    
    # get my base
    if team_color == 'r':
        my_base = game_state["PlayerBaseR"]
    else:
        my_base = game_state["PlayerBaseB"]
    ...
    for d in queue_directions:
        dx, dy = dir_offsets[d]
        spawn_x = base_x + dx
        spawn_y = base_y + dy
        
        # simple heuristic: go in direction that minimizes distance to enemy base
        dist = abs(spawn_x - opponentBase_x) + abs(spawn_y - opponentBase_y)
        
        # slight randomness tie-breaker to avoid always same lane
    ...
    # fallback, should never be empty if queue_directions not empty
    if best_dir == "" and len(queue_directions) > 0:
        best_dir = random.choice(queue_directions)
...
class Agent:
    ...
    def do_turn(self, game_state: dict) -> AIAction:
        ...
        best_dir = get_best_queue_direction(game_state, self.team_color, queue_direction, opponentBase_x, opponentBase_y)
